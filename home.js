...
function canWeDeliver(zipCode)
{
    ...
    {
    //console.log(`sorry we cant delicet to ${zipCode}`)
    }
    console.log(`sorry we cant delicet to ${zipCode}`)
}
...
function canWeDeliverTwo(zipCode)
{
    ...
    let zip = (deliveryAreaZipCodes.includes(zipCode));
    if (zip === false)
    console.log(`No delivery to ${zip}`)
    else 
    console.log("Again You are in our delivery area")
...
//deals.splice(0,0,"10 OFF")

//deals[0].title = "10% OFF"

const newDiscount = deals.filter((arrayObj) =>{
    if(arrayObj.title === "15% Off!"){
    arrayObj.title = "10 OFF"
    console.log(`Discount ${arrayObj.title}`)
    }
...
const newMonthDiscount = deals.filter((arrayObj) =>{
    if(arrayObj.desc.includes("March")){
    arrayObj.desc = "This deal lasts until the end of April!"
    console.log(`Discount ${arrayObj.title}`)
